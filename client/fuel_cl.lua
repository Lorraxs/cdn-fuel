...
if Config.FuelDebug then
	RegisterCommand('setfuel', function(source, args)
		...
		CORE.ShowNotification(Lang:t("set_fuel_debug") .. ' ' .. args[1] .. 'L', 'success')
	end, false)
...
local function FetchStationInfo(info)
	...
	local result = lib.callback.await('cdn-fuel:server:fetchinfo', 400, CurrentLocation)
	if result then
		for _, v in pairs(result) do
			...
			if info == "all" or info == "reserves" then
				...
				if Config.UnlimitedFuel then
					ReservesNotBuyable = true
					if Config.FuelDebug then print("Reserves are not buyable, because Config.UnlimitedFuel is set to true.") end
				end
			end
			-- Fuel Price --
...
local function CanAfford(price, purchasetype)
	...
	if purchasetype == "bank" then
		Money = CORE.PlayerData.data.money['bank']
	elseif purchasetype == 'cash' then
		Money =
				CORE.PlayerData.data.money['cash']
	end
	if Money < price then
		return false
	else
...
if Config.ShowNearestGasStationOnly then
	...
	CreateThread(function()
		...
		for i = 1, #Config.GasStations, 1 do
			local location = i
			gasStationCoords = vector3(Config.GasStations[location].pedcoords.x, Config.GasStations[location].pedcoords.y,
				Config.GasStations[location].pedcoords.z)
...
if Config.RenewedPhonePayment then
	RegisterNetEvent('cdn-fuel:client:phone:PayForFuel', function(amount)
		...
		if Config.EmergencyServicesDiscount['enabled'] == true then
			...
			local plyJob = CORE.PlayerData.job.name
			...
			if shouldRecieveDiscount == true and not CORE.PlayerData.job.onduty and Config.EmergencyServicesDiscount['ondutyonly'] then
				CORE.ShowNotification(Lang:t("you_are_discount_eligible"), 'primary', 7500)
					...
					if discount == 100 then
						...
						if Config.FuelDebug then
							print("Your discount for Emergency Services is set @ " .. discount ..
								"%. Setting new price to: $" .. FuelPrice)
						end
					end
				else
					if Config.FuelDebug then
		...
		local success = exports['qb-phone']:PhoneNotification(Lang:t("fuel_phone_header"),
			...
			'fas fa-times-circle')
		if success then
			if CORE.PlayerData.data.money['bank'] <= total then
				CORE.ShowNotification(Lang:t("not_enough_money"), "error")
			else
		...
		if not RefuelPossible then
			...
			if not Cancelledrefuel and not RefuelCancelled then
				if RefuelPossibleAmount then
					...
					CORE.ShowNotification(Lang:t("more_than_zero"), 'error', 7500)
				end
			end
		end
	else
...
RegisterNetEvent('cdn-fuel:client:grabnozzle', function()
	if Config.PlayerOwnedGasStationsEnabled then
		...
		local result = lib.callback.await('cdn-fuel:server:checkshutoff', 400, CurrentLocation)
		if result then
			if result == true then
				CORE.ShowNotification(Lang:t("emergency_shutoff_active"), 'error', 7500)
				ShutOff = true
			else
				ShutOff = false
			end
	...
	if not ShutOff then
		...
		if Config.PumpHose then
			...
			AttachEntitiesToRope(Rope, pump, fuelnozzle, pumpCoords.x, pumpCoords.y, pumpCoords.z + PumpHeightAdd, nozzlePos.x,
				nozzlePos.y, nozzlePos.z, length, false, false, nil, nil)
			if Config.FuelDebug then
				print("Hose Properties:")
				print(Rope, pump, fuelnozzle, pumpCoords.x, pumpCoords.y, pumpCoords.z, nozzlePos.x, nozzlePos.y, nozzlePos.z,
					length)
		...
		CreateThread(function()
			while holdingnozzle do
				...
				if dist > 7.5 then
					...
					CORE.ShowNotification(Lang:t("nozzle_cannot_reach"), 'error')
					if Config.PumpHose == true then
						RopeUnloadTextures()
						DeleteRope(Rope)
					end
					if Config.FuelNozzleExplosion then
						AddExplosion(grabbednozzlecoords.x, grabbednozzlecoords.y, grabbednozzlecoords.z, 'EXP_TAG_PROPANE', 1.0,
							true, false, 5.0)
	...
	if RefuelingType == nil then
		...
		if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
			if ReserveLevels < 1 then
				CORE.ShowNotification(Lang:t("station_no_fuel"), 'error', 7500)
				return
			end
		end
		if Config.PlayerOwnedGasStationsEnabled then
		...
		if shouldRecieveDiscount then
			...
			if discount ~= 0 then
				if discount == 100 then
					...
					discount = discount / 100
					if Config.FuelDebug then
						print("Math( Current Fuel Price: " .. FuelPrice ..
							" - " .. FuelPrice * discount .. "<<-- FuelPrice * Discount)")
					end
					CachedFuelPrice = FuelPrice
					FuelPrice = (FuelPrice) - (FuelPrice * discount)
					if Config.FuelDebug then
						print("Your discount for Emergency Services is set @ " .. discount .. "%. Setting new price to: $" ..
							FuelPrice)
					end
				end
			else
	...
	if Config.Ox.Input then
		...
		if fuel then
			...
			if not holdingnozzle and RefuelingType ~= 'special' then
				CORE.ShowNotification(Lang:t("no_nozzle"), 'error')
				return
			end
			if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == "special" then
				if tonumber(fuelAmount) > tonumber(ReserveLevels) then
					CORE.ShowNotification(Lang:t("station_not_enough_fuel"), "error")
					return
				end
			end
			if (fuelAmount + finalfuel) >= 100 then
				CORE.ShowNotification(Lang:t("tank_cannot_fit"), "error")
			else
				if GlobalTax(fuelAmount * FuelPrice) + (fuelAmount * FuelPrice) <= money then
					...
					CORE.ShowNotification(Lang:t("not_enough_money"), 'error', 7500)
				end
			end
		else
			if Config.FuelDebug then
		...
		if fuel then
			...
			if not holdingnozzle and RefuelingType ~= 'special' then
				CORE.ShowNotification(Lang:t("no_nozzle"))
				return
			end
			if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == 'special' then
...
RegisterNetEvent('cdn-fuel:client:SendMenuToServer', function(type)
	...
	if Config.ElectricVehicleCharging then
		...
		if isElectric == 'electricvehicle' then
			CORE.ShowNotification(Lang:t("need_electric_charger"), 'error', 7500)
			return
		end
		NotElectric = true
	else
	...
	if NotElectric then
		...
		local playercashamount = CORE.PlayerData.data.money['cash']
		...
		if CurFuel < 95 then
			...
			CORE.ShowNotification(Lang:t("tank_already_full"), 'error')
		end
	else
		CORE.ShowNotification(Lang:t("need_electric_charger"), 'error', 7500)
	end
			...
			CreateThread(function()
				while refueling do
					...
					if Cancelledrefuel then
						...
						if RefuelingType == nil then
							if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
								TriggerServerEvent('cdn-fuel:station:server:updatereserves', "remove", finalrefuelamount, ReserveLevels,
									CurrentLocation)
								if CachedFuelPrice ~= nil then
									if Config.FuelDebug then
										print("We have a cached price: $" .. CachedFuelPrice .. ", we will credit this to the gas station.")
									end
									TriggerServerEvent('cdn-fuel:station:server:updatebalance', "add", finalrefuelamount, StationBalance,
...
RegisterNetEvent('cdn-fuel:jerrycan:refuelmenu', function(itemData)
	if IsPedInAnyVehicle(PlayerPedId(), false) then
		CORE.ShowNotification(Lang:t("cannot_refuel_inside"), 'error')
		return
	end
	...
	if GetVehicleBodyHealth(vehicle) < 100 then
		CORE.ShowNotification(Lang:t("vehicle_is_damaged"), 'error')
		return
	end
...
RegisterNetEvent('cdn-fuel:client:jerrycanfinalmenu', function(purchasetype)
	Moneyamount = nil
	if purchasetype == 'bank' then
		Moneyamount = CORE.PlayerData.data.money['bank']
	elseif purchasetype == 'cash' then
		Moneyamount = CORE.PlayerData.data.money['cash']
	end
	if Moneyamount > math.ceil(Config.JerryCanPrice + GlobalTax(Config.JerryCanPrice)) then
		...
		if purchasetype == 'bank' then CORE.ShowNotification(Lang:t("not_enough_money_in_bank"), 'error') end
		if purchasetype == "cash" then CORE.ShowNotification(Lang:t("not_enough_money_in_cash"), 'error') end
	end
end)
		...
		if Config.Ox.Input then
			...
			if refuel and refuelAmount then
				if tonumber(refuelAmount) == 0 then
					CORE.ShowNotification(Lang:t("more_than_zero"), 'error')
					return
				...
				if tonumber(refuelAmount) > jerrycanfuelamount then
					CORE.ShowNotification(Lang:t("jerry_can_not_enough_fuel"), 'error')
					return
				end
				...
				if Config.Ox.Progress then
					if lib.progressCircle({
						...
						CORE.ShowNotification(Lang:t("jerry_can_success_vehicle"), 'success')
						local JerryCanItemData = data.itemData
						local srcPlayerData = CORE.PlayerData.data
						...
						DeleteObject(JerrycanProp)
						StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
						CORE.ShowNotification(Lang:t("cancelled"), 'error')
					end
				else
	...
	if Config.Ox.Input then
		...
		if refuel then
			...
			if tonumber(refuelAmount) + tonumber(jerrycanfuelamount) > Config.JerryCanCap then
				CORE.ShowNotification(Lang:t("jerry_can_not_fit_fuel"), 'error')
				return
			end
			...
			if not CanAfford(price, "cash") then
				CORE.ShowNotification(Lang:t("not_enough_money_in_cash"), 'error')
				return
			end
			...
			if lib.progressCircle({
				...
				DeleteObject(JerrycanProp)
				StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
				CORE.ShowNotification(Lang:t("jerry_can_success"), 'success')
				local srcPlayerData = CORE.PlayerData.data
				if Config.Ox.Inventory then
				...
				if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
					TriggerServerEvent('cdn-fuel:station:server:updatereserves', "remove", tonumber(refuelAmount), ReserveLevels,
						CurrentLocation)
					if CachedFuelPrice ~= nil then
						TriggerServerEvent('cdn-fuel:station:server:updatebalance', "add", tonumber(refuelAmount), StationBalance,
							CurrentLocation, CachedFuelPrice)
					else
						TriggerServerEvent('cdn-fuel:station:server:updatebalance', "add", tonumber(refuelAmount), StationBalance,
							CurrentLocation, FuelPrice)
...
RegisterNetEvent('cdn-syphoning:syphon:menu', function(itemData)
	if IsPedInAnyVehicle(PlayerPedId(), false) then
		CORE.ShowNotification(Lang:t("syphon_inside_vehicle"), 'error')
		return
	end
	...
	if Config.ElectricVehicleCharging then
		...
		if Config.ElectricVehicles[vehiclename] and Config.ElectricVehicles[vehiclename].isElectric then
			...
			CORE.ShowNotification(Lang:t("syphon_electric_vehicle"), 'error', 7500)
			return
		end
	else
		NotElectric = true
...
RegisterNetEvent('cdn-syphoning:syphon', function(data)
	...
	if NotElectric then
		...
		if HasSyphon then
				...
				if Config.Ox.Input then
					...
					if syphon then
						...
						if tonumber(syphonAmount) < 0 then
							CORE.ShowNotification(Lang:t("syphon_more_than_zero"), 'error')
							return
						end
						if tonumber(syphonAmount) == 0 then
							CORE.ShowNotification(Lang:t("syphon_more_than_zero"), 'error')
						...
						if tonumber(syphonAmount) > maxsyphon then
							CORE.ShowNotification(Lang:t("syphon_kit_cannot_fit_1") .. fitamount .. Lang:t("syphon_kit_cannot_fit_2"),
								'error')
							return
						end
						...
						if (tonumber(syphonAmount) <= tonumber(cargasamount)) then
							...
							if lib.progressCircle({
								...
								if GetFuel(vehicle) >= syphonAmount then
									...
									CORE.ShowNotification(Lang:t("syphon_success"), 'success')
									...
									CORE.ShowNotification(Lang:t("menu_syphon_vehicle_empty"), 'error')
								end
							else
								PoliceAlert(GetEntityCoords(ped))
								StopAnimTask(ped, Config.StealAnimDict, Config.StealAnim, 1.0)
				...
				if Config.Ox.Input then
					...
					if refuel then
						...
						elseif tonumber(refuelAmount) > 100 then
							CORE.ShowNotification("You can't refuel more than 100L!", 'error')
							return
						end
						if tonumber(refuelAmount) > tonumber(currentsyphonamount) then
							CORE.ShowNotification(Lang:t("syphon_not_enough_gas"), 'error')
							return
						end
						...
						if lib.progressCircle({
							...
							StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
							CORE.ShowNotification(Lang:t("syphon_success_vehicle"), 'success')
			...
			CORE.ShowNotification(Lang:t("syphon_no_syphon_kit"), 'error', 7500)
		end
	else
		CORE.ShowNotification(Lang:t("need_electric_charger"), 'error', 7500)
		return
...
RegisterNetEvent('cdn-syphoning:client:callcops', function(coords)
	local PlayerJob = CORE.PlayerData.job
...
RegisterNetEvent('cdn-fuel:client:grabnozzle:special', function()
	...
	CORE.ShowNotification(Lang:t("show_input_key_special"))
	if Config.PumpHose then
		local pumpCoords, pump = GetClosestPump(grabbednozzlecoords)
		-- Load Rope Textures
		RopeLoadTextures()
		...
		AttachEntitiesToRope(Rope, pump, SpecialFuelNozzleObj, pumpCoords.x, pumpCoords.y, pumpCoords.z + 2.1, nozzlePos.x,
			nozzlePos.y, nozzlePos.z, length, false, false, nil, nil)
	...
	CreateThread(function()
		while HoldingSpecialNozzle do
			...
			if dist > Config.AirAndWaterVehicleFueling['nozzle_length'] or IsPedInAnyVehicle(ped, false) then
				...
				if Config.FuelNozzleExplosion then
					AddExplosion(grabbednozzlecoords.x, grabbednozzlecoords.y, grabbednozzlecoords.z, 'EXP_TAG_PROPANE', 1.0, true,
						false, 5.0)
					StartScriptFire(grabbednozzlecoords.x, grabbednozzlecoords.y, grabbednozzlecoords.z - 1, 25, false)
					SetFireSpreadRate(10.0)
					Wait(5000)
		...
		if LocalPlayer.state['isLoggedIn'] then
			for i = 1, #Config.AirAndWaterVehicleFueling['locations'], 1 do
				...
				AirSeaFuelZones[k].PolyZone:onPlayerInOut(function(isPointInside)
					if isPointInside then
						...
						if Config.AirAndWaterVehicleFueling['locations'][i]['whitelist']['enabled'] then
							...
							local plyJob = CORE.PlayerData.job
								...
								while PlayerInSpecialFuelZone do
									...
									if IsControlJustReleased(0, Config.AirAndWaterVehicleFueling['refuel_button']) --[[ Control in Config ]] then
										...
										if not HoldingSpecialNozzle then
											CORE.ShowNotification(Lang:t("no_nozzle"), 'error', 1250)
										elseif dist > 4.5 then
											CORE.ShowNotification(Lang:t("vehicle_too_far"), 'error', 1250)
										elseif IsPedInAnyVehicle(PlayerPedId(), true) then
											CORE.ShowNotification(Lang:t("inside_vehicle"), 'error', 1250)
										else
				...
				if currentLocation['prop'] then
					...
					if Config.FuelDebug then
						print("Location #" ..
							i .. " for Special Fueling Zones (Air and Sea) doesn't have a prop set up, so players cannot fuel here.")
					end
				end
...
AddEventHandler("QBCore:Client:OnPlayerLoaded", function()
	for i = 1, #Config.AirAndWaterVehicleFueling['locations'], 1 do
		...
		-- Grab MinZ & MaxZ from Config.
		local minimumZ, maximumZ = currentLocation['PolyZone']['minmax']['min'], currentLocation['PolyZone']['minmax']
				['max']
				...
				if not IsVehicleBlacklisted(vehPedIsIn) then
					...
					if vehFuelLevel <= vehFuelShutoffLevel then
						if GetIsVehicleEngineRunning(vehPedIsIn) then
							...
							CORE.ShowNotification(Lang:t("no_fuel"), 'error', 3500)
